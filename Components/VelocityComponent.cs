...
public partial class VelocityComponent : Node2D
{
	...
	// [Export]
	// private bool useMoveAndCollide;
	[Export]
	...
	public void MoveWithCollision(CharacterBody2D characterBody, double delta)
	{
		...
		collision = characterBody.MoveAndCollide(characterBody.Velocity * (float)delta);
		if (collision != null)
		{
			//GD.Print("[VelocityComponent/MoveCollision] ", collision);
			EmitSignal(SignalName.KinematicCollision, collision.GetCollider());
		}
	...
	public void Bounce(CharacterBody2D characterBody)
	{
		...
		Velocity = characterBody.Velocity.Bounce(collision.GetNormal());
	}
