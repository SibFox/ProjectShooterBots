@tool
class_name Cam extends CamBase
...
func _update_zoom(delta: float) -> void:
	...
	#_current_zoom = 1920 / get_viewport_rect().size.x * zoom
	
	#_camera.zoom.x = ProjectSettings.get("display/window/size/viewport_width") / get_viewport_rect().size.x * zoom
	#_camera.zoom.y = ProjectSettings.get("display/window/size/viewport_height") / get_viewport_rect().size.y * zoom
...
func _calculate_deadzone_rect() -> Rect2:
	...
	var offset_left = half_width * (1.0 - left_margin)
	var offset_top = half_height * (1.0 - top_margin)
	var offset_right = half_width * (1.0 - right_margin)
	var offset_bottom = half_height * (1.0 - bottom_margin)

	var rect = Rect2(
		-half_width + offset_left,
		-half_height + offset_top,
		size.x - offset_left - offset_right,
		size.y - offset_top - offset_bottom
	)
	return rect
