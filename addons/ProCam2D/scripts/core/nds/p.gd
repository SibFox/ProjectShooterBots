...
@export var constraint_axis: AxisConstraint = AxisConstraint.Y
...
func _ready():
	connect("child_entered_tree", Callable(self, "update_configuration_warnings"))
	_update_path()
...
func apply_constraint(camera_pos: Vector2) -> Vector2:
	...
	match constraint_axis:
		AxisConstraint.X:
			...
			_pos2d.position = _path.curve.interpolate_baked(closest_offset, false)
...
func _draw_debug():
	...
	for i in range(1, points.size()):
		draw_line(points[i-1], points[i], debug_color[1], 2)
